...
import type { ShellBackend, ShellLoadPhase } from '../lib/shellRuntime.ts'
...
export function ShellRuntimeBanner({ phase, backend, message, error, onRetry }: Props) {
  ...
  const runtimeLabel =
    backend === 'v86' ? 'v86 Podman VM' : backend === 'wasmer' ? 'Wasmer shell' : 'shell runtime'
    ...
    <div className={`runtime-banner runtime-banner--${state}`} role="status" aria-live="polite">
      ...
      <div className="runtime-text">
        {phase === 'unsupported' ? (
          ...
          <span>
            <strong>Loading {runtimeLabel}…</strong> {message}
          </span>
        )}
      </div>
...
/** @deprecated Use ShellRuntimeBanner */
export const WasmerRuntimeBanner = ShellRuntimeBanner
