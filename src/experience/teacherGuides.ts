...
export const teacherGuides: Record<string, TeacherGuide> = {
  ...
  sql: {
    ...
    lab: 'Run LEFT JOIN, HAVING, subquery, and INSERT samples; compare revenue per city with the MongoDB lesson aggregation.',
    assess: [
  ...
  mongodb: {
    ...
    lab: 'Run aggregate revenue by city, insert a new order, update pending orders, and compare with the SQL GROUP BY lesson.',
    assess: [
      'Write a find query for orders over $100 in London.',
      'Explain embed vs reference with an example.',
      'What does $group with $sum do in a pipeline?',
      'In MongoDB, a document is best described as:',
      'updateMany with $set:',
    ],
  },
  regex: {
  ...
  fastapi: {
    objectives: [
      ...
      'Explain dependency injection with Depends for shared logic.',
    ...
    lab: 'Run Route handlers and trace 404 vs 200; validate bodies in Pydantic-style validation; simulate DI and raise HTTPException for a missing user.',
    assess: [
  ...
  react: {
    ...
    keyConcepts: [
      ...
      'Virtual DOM diffing minimizes real DOM updates.',
    ],
    misconceptions: [
      ...
      '"You need Redux for any state."',
    ],
    discussion: [
      'When should state be lifted to a parent vs kept local?',
      'What problems do missing keys in lists cause?',
      'How does React pair with FastAPI in a full-stack app?',
    ],
    lab: 'Use the playground: increment the counter, edit greeting props, add/toggle todos. Relate each interaction to useState or props.',
    assess: [
      'What is the difference between props and state?',
      'Why use setTodos(t => t.map(...)) instead of mutating the array?',
      'What does an empty useEffect dependency array mean?',
      'JSX compiles to:',
    ],
  },
  security: {
